...
implist = demo_and_response_data(onedrive_wd = onedrive_wd, return_implist = T) 
M = length(implist)-1
...
implist = lapply(0:M, function(m){
  ...
  dat_m = dat_m %>% 
    dplyr::select(imp, rid,name,varshort,essential,category,role_primary, gender, edu_years:Simulationist) %>% 
    dplyr::mutate(across(edu_years:age_years, .fns = scale))
...
# Null model
mira_0 = fit_glmer(formula = essential ~ category + (1|name) + (1|varshort), 
                   implist = implist, optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3)
                   )
# role_primary
mira_role_primary = fit_glmer(essential ~ role_primary*category + (1|name) + (1|varshort), implist = implist, optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
pval_role_primary = summary(D1(mira_role_primary, mira_0))$comparisons$p.value
# gender
mira_gender = fit_glmer(essential ~ gender*category + (1|name) + (1|varshort), implist = implist, optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
pval_gender = summary(D1(mira_gender, mira_0))$comparisons$p.value
# age_years
mira_age_years = fit_glmer(essential ~ age_years*category + (1|name) + (1|varshort), implist = implist,  optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
pval_age_years = summary(D1(mira_age_years, mira_0))$comparisons$p.value
# edu_years
mira_edu_years = fit_glmer(essential ~ edu_years*category + (1|name) + (1|varshort), implist = implist,  optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
pval_edu_years = summary(D1(mira_edu_years, mira_0))$comparisons$p.value
# rn_years
mira_rn_years = fit_glmer(essential ~ rn_years*category + (1|name) + (1|varshort), implist = implist,  optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
pval_rn_years = summary(D1(mira_rn_years, mira_0))$comparisons$p.value
# expertise
mira_expertise = fit_glmer(essential ~ Medical_Surgical*category + Population_Health*category + Behavioral_Health*category + Critical_Car*category + ED*category + Perioperative*category + OB*category+ Pediatrics*category + (1|name) + (1|varshort), implist = implist,  optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
pval_expertise = summary(D1(mira_expertise, mira_0))$comparisons$p.value
# role_current
mira_role_current = fit_glmer(essential ~ New_Grad_Res_Coord_Educator*category + Clinical_Instructor_Academic*category + Preceptor*category + Simulationist*category + (1|name) + (1|varshort), implist = implist,  optimizer = c("bobyqa", "nlminbwrap"), optCtrl = list(eval.max = 2E7, iter.max = 5E3))
...
hi = with(mids_, glmer(essential ~ gender + (1|name) + (1|varshort), family = "binomial", verbose = 1), parallel = "multisession")
class(hi)
pool(hi)
...
# role_primary (significant)
  fit_role_primary0 = lme4::glmer(
    essential ~ (1|name) + (1|varshort), 
    data = implist[[1]] %>% dplyr::select(essential, name,varshort, role_primary) %>% na.omit(), 
    family = "binomial", 
    verbose = 1, 
    control = glmerControl(optimizer = "bobyqa")
  )
  fit_role_primary1 = lme4::glmer(
    essential ~ role_primary + (1|name) + (1|varshort), 
    data = implist[[1]] %>% dplyr::select(essential, name,varshort, role_primary) %>% na.omit(), 
    family = "binomial", 
    verbose = 1, 
    control = glmerControl(optimizer = "bobyqa")
  )
  lrt_role_primary_01 = lmtest::lrtest(fit_role_primary0,fit_role_primary1) %>%
    data.frame() %>%
    dplyr::slice(-1) %>%
    dplyr::mutate(Comparison = "M0 vs. M1", Grouping = "role_primary")
  

# gender (n.s.)
  fit_role_gender0 = lme4::glmer(
    essential ~ (1|name) + (1|varshort), 
    data = implist[[1]] %>% dplyr::select(essential, name,varshort, gender) %>% na.omit(), 
    family = "binomial", 
    verbose = 1, 
    control = glmerControl(optimizer = "bobyqa")
  )
  fit_role_gender1 = lme4::glmer(
    essential ~ gender + (1|name) + (1|varshort), 
    data = implist[[1]] %>% dplyr::select(essential, name,varshort, gender) %>% na.omit(), 
    family = "binomial", 
    verbose = 1, 
    control = glmerControl(optimizer = "bobyqa")
  )
  lrt_gender_01 = lmtest::lrtest(fit_role_gender0,fit_role_gender1) %>%
    data.frame() %>%
    dplyr::slice(-1) %>%
    dplyr::mutate(Comparison = "M0 vs. M1", Grouping = "gender")

  
# age_years (n.s.)
  fit_role_age_years0 = lme4::glmer(
    essential ~ (1|name) + (1|varshort), 
    data = implist[[1]] %>% dplyr::select(essential, name,varshort, age_years) %>% na.omit(), 
    family = "binomial", 
    verbose = 1, 
    control = glmerControl(optimizer = "bobyqa")
  )
  fit_role_age_years1 = lme4::glmer(
    essential ~ age_years + (1|name) + (1|varshort), 
    data = implist[[1]] %>% dplyr::select(essential, name,varshort, age_years) %>% na.omit(), 
    family = "binomial", 
    verbose = 1, 
    control = glmerControl(optimizer = "bobyqa")
  )
  lrt_age_years_01 = lmtest::lrtest(fit_role_age_years0,fit_role_age_years1) %>%
    data.frame() %>%
    dplyr::slice(-1) %>%
    dplyr::mutate(Comparison = "M0 vs. M1", Grouping = "age_years")
  
  
  

# Main effect models
mids_df = implist %>% dplyr::bind_rows() %>% mice::as.mids(.imp = "imp", .id = "rid")

fit0 = mids_df %>% 
  with(glmer(essential ~ role_primary + (1|name) + (1|varshort), family = "binomial", verbose = 1, control = glmerControl(optimizer = "bobyqa")))
pool(fit0)
...
fit0 = lme4::glmer(
  essential ~ role_primary + (1|name) + (1|varshort), 
  data = implist[[2]] %>% dplyr::select(essential, name,varshort, role_primary), 
  family = "binomial", 
  verbose = 1, 
  control = glmerControl(optimizer = "bobyqa")
)